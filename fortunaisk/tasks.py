...
def process_payment(payment_locked):
    ...
    try:
        ...
        logger.debug(
            f"CharacterOwnership found for character ID {eve_character.character_id}."
        )
        ...
        ProcessedPayment.objects.create(
            payment_id=payment_id, processed_at=timezone.now()
        )
        return
    ...
    logger.debug(
        f"Payment ID {payment_id}: lottery_reference='{lottery_reference}', amount={payment_amount}"
    )

    # Step 3: Retrieve the active lottery.
    try:
        lottery = Lottery.objects.select_for_update().get(
            lottery_reference=lottery_reference, status__in=["active", "pending"]
        )
    ...
    if not (lottery.start_date <= payment_date <= lottery.end_date):
        logger.warning(
            f"Payment date {payment_date} is outside the lottery period ({lottery.start_date} - {lottery.end_date}) for lottery '{lottery.lottery_reference}'."
        )
        TicketAnomaly.objects.create(
            lottery=lottery,
            user=user,
    ...
    if num_tickets_possible < 1:
        logger.warning(
            f"Payment amount {payment_amount} ISK is insufficient for even one ticket at {ticket_price} ISK for lottery '{lottery.lottery_reference}'."
        )
        TicketAnomaly.objects.create(
            lottery=lottery,
            user=user,
    ...
    allowed_tickets = (
        ...
        else num_tickets_possible
    )
    logger.debug(
        f"User '{user.username}' currently has {current_tickets} tickets for lottery '{lottery.lottery_reference}'. Allowed additional tickets: {allowed_tickets}."
    )
    if lottery.max_tickets_per_user and allowed_tickets < 1:
        logger.warning(
            f"User '{user.username}' has reached the maximum number of tickets ({lottery.max_tickets_per_user}) for lottery '{lottery.lottery_reference}'."
        )
        TicketAnomaly.objects.create(
            lottery=lottery,
            user=user,
    ...
    if remainder > 0:
        logger.warning(
            f"Payment ID {payment_id}: Overpayment detected. Remainder: {remainder} ISK not applied to ticket purchase."
        )
        TicketAnomaly.objects.create(
            lottery=lottery,
            user=user,
...
def process_payment_task(self, payment_entry_id):
    ...
    try:
        with transaction.atomic():
            payment_locked = CorpJournal.objects.select_for_update().get(
                entry_id=payment_entry_id
            )
            process_payment(payment_locked)
    except Exception as e:
        logger.error(
            f"Error in process_payment_task for payment_id {payment_entry_id}: {e}",
            exc_info=True,
        )
        raise e
...
def check_purchased_tickets(self):
    ...
    try:
        ...
        tasks = [
            process_payment_task.s(payment.entry_id) for payment in pending_payments
        ]
        if tasks:
        ...
        logger.critical(
            f"General error in 'check_purchased_tickets' task: {outer_e}", exc_info=True
        )
...
def wait_for_corp_audit_update():
    ...
    try:
        ...
        while total_wait < max_wait:
            ...
            total_wait += 10
        logger.warning(
            "Timeout reached waiting for 'Corporation Audit Update'. Proceeding anyway."
        )
    except Exception as e:
        logger.warning(
            f"Could not retrieve 'Corporation Audit Update' periodic task: {e}. Proceeding immediately."
        )
...
def check_lottery_status(self):
    ...
    try:
        ...
        active_lotteries = Lottery.objects.filter(
            status__in=["active", "pending"], end_date__lte=now
        )
        ...
        for lottery in active_lotteries:
            ...
            logger.info(
                f"Lottery '{lottery.lottery_reference}' (ID {lottery.id}) set to 'pending'."
            )
            ...
            processed_payments_qs = CorpJournal.objects.filter(
                entry_id__in=ProcessedPayment.objects.values_list(
            ...
            if processed_payments_qs.exists():
                last_processed_date = (
                    processed_payments_qs.order_by("-date").first().date
                )
            else:
            ...
            if pending_payments.exists():
                count = pending_payments.count()
                logger.info(
                    f"Lottery '{lottery.lottery_reference}' (ID {lottery.id}) has {count} pending payment(s) after {last_processed_date}. Launching check_purchased_tickets synchronously."
                )
            ...
            TicketPurchase = apps.get_model("fortunaisk", "TicketPurchase")
            total_data = TicketPurchase.objects.filter(lottery=lottery).aggregate(
                total=Sum("amount")
            )
            ...
            logger.info(
                f"Updated total pot for lottery '{lottery.lottery_reference}' to {lottery.total_pot} ISK before finalization."
            )

            # 6. Finalisation : si le pot est > 0, tenter de sélectionner les gagnants.
            if lottery.total_pot == 0:
                logger.error(
                    f"Total pot is 0 for lottery {lottery.lottery_reference}. Cannot select winners."
                )
            else:
...
def setup_periodic_tasks():
    ...
    try:
        ...
        logger.info(
            "Periodic task 'check_purchased_tickets' updated/created successfully."
        )
        ...
        logger.info(
            "Periodic task 'check_lottery_status' updated/created successfully."
        )
